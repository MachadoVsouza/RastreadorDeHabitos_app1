...
class _HabitEditScreenState extends ConsumerState<HabitEditScreen> {
  ...
  void initState() {
    super.initState();
    final habit = ref.read(habitProvider.notifier).getHabitById(widget.habitId);
    _habit = habit;
    if (habit != null) {
      _titleController.text = habit.title;
      _descriptionController.text = habit.description ?? '';
      _selectedDays.addAll(habit.daysOfWeek);
      _time = AppDateUtils.parseTime(habit.time);
      _notifications = habit.notificationEnabled;
  ...
  Widget build(BuildContext context) {
    if (_habit == null) {
      return Scaffold(
        ...
        body: const Center(child: Text('Hábito não encontrado')),
      );
    }
